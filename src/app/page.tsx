...
function AiSearchOverlay() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        // You can pass a parameter here to get more recommendations if your Worker supports it
        // e.g. { query: userQuery, maxRecommendations: 5 }
        const res = await fetch("https://cold-bush-ec7b.pauljash.workers.dev/", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ query: userQuery, maxRecommendations: 5 }),
        });
        if (!res.ok) throw new Error(`Request failed with status ${res.status}`);

        const data = await res.json();
        // Expecting data.recommendations as an array of objects now
        if (!Array.isArray(data.recommendations)) {
          throw new Error("Unexpected API response format");
        }
        // Each item should have at least { name, image? } structure
      ...
      <AnimatePresence>
        {(userQuery || isLoading || errorMessage) && (
          ...
          >
            ...
            {/* If we have object-based recommendations, display with thumbnail */}
